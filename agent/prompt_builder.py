...
def _collect_extra_skills(
    ...
    *, desc_prefix: str, log_fmt: str, visible_entries: list[dict] | None = None,
) -> None:
...
def _build_skills_system_prompt_inner(
    ...
    if project_dirs:
        ...
        for proj_dir in (d for d in project_dirs if d.exists()):
            _collect_extra_skills(proj_dir, iter_project_skill_files(proj_dir), hides, project_names, skills_by_category,
                                  ...
                                  visible_entries=project_entries)
    ...
    visible_entries.extend(project_entries)
    ...
    if prominent_catalog and result:
        result = result.replace("<available_skills>", prominent_catalog + "<available_skills>", 1)
    with _SKILLS_PROMPT_CACHE_LOCK:
