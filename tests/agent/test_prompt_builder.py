...
class TestBuildSkillsSystemPrompt:
    ...
    def test_prominent_roots_follow_project_precedence_and_external_visibility(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setenv("HERMES_HOME", str(tmp_path))
        (tmp_path / "config.yaml").write_text(
            "skills:\n  prominent_roots: [shared, external, hidden]\n  disabled: [hidden]\n",
            encoding="utf-8",
        )
        local, project, external = (tmp_path / name for name in ("skills", "project-skills", "external-skills"))
        for root, name, description in (
            (local, "shared", "LOCAL-ROOT"),
            (project, "shared", "PROJECT-ROOT"),
            (external, "external", "EXTERNAL-ROOT"),
            (external, "hidden", "HIDDEN-ROOT"),
        ):
            skill = root / name
        ...
        result = pb._build_skills_system_prompt_inner(local, [external], None, None, None, [project])
        catalog = result.split("## Prominent skills\n", 1)[1].split("\n\n", 1)[0]
        assert "PROJECT-ROOT" in catalog and "EXTERNAL-ROOT" in catalog
        assert "LOCAL-ROOT" not in result and "HIDDEN-ROOT" not in result
        assert "[project] PROJECT-ROOT" in result
        # Exercise the persisted snapshot, not only the in-process LRU.
        pb._SKILLS_PROMPT_CACHE.clear()
        assert pb._build_skills_system_prompt_inner(local, [external], None, None, None, [project]) == result
